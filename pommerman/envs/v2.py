...
class Pomme(v0.Pomme):
    ...
    def get_json_info(self):
        ...
        ret['radio_vocab_size'] = json.dumps(
            self._radio_vocab_size, cls=utility.PommermanJSONEncoder)
        ret['radio_num_words'] = json.dumps(
            self._radio_num_words, cls=utility.PommermanJSONEncoder)
        json_radio_from_agent = {key.value: value for (key, value) in self._radio_from_agent.items()}
        ret['_radio_from_agent'] = json.dumps(
            json_radio_from_agent, cls=utility.PommermanJSONEncoder)
        return ret
    ...
    def set_json_info(self):
        ...
        json_radio_from_agent = json.loads(
            self._init_game_state['_radio_from_agent'])
        self._radio_from_agent = {constants.Item[constants.Item(key)]: value for (key, value) in json_radio_from_agent}
